...
function calGoal(p) {
  ...
  const adj = {
    'Recomposición corporal': 0,
    'Definición / Cutting': -300,
    'Volumen / Bulking': 300,
    'Mantenimiento': 0,
    'Pre-competencia': -500,
  }
...
function getGreeting(name) {
  ...
  if (h < 12) return `Ave, ${name}`
  if (h < 20) return `Salve, ${name}`
  return `Vesper, ${name}`
}
...
function Panel({ children, style = {} }) {
  return (
    <div style={{ background: C.panel, borderRadius: 12, padding: 16, marginBottom: 12, border: `1px solid ${C.stone}`, ...style }}>
      {children}
    </div>
  )
}
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      <input type={type} value={value} onChange={e => onChange(type === 'number' ? (parseFloat(e.target.value) || 0) : e.target.value)}
        ...
        style={{ width: '100%', background: C.stone, border: `1px solid ${C.gold}40`, borderRadius: 6, color: C.marble, padding: '8px 12px', fontFamily: 'Inter,sans-serif', fontSize: 13 }} />
    </div>
  )
}
...
function TabHoy({ profile, foods, water, setWater, activities, setActivities, weights, setWeights }) {
  ...
  let barColor = C.gold
  if (balance > 150) barColor = C.red
  else if (balance < -150) barColor = C.blue

  async function consultOracle() {
    setOracleLoading(true); setOracleMsg('')
    try {
      const h = new Date().getHours()
      const ctx = `Hora: ${h}:00h. Kcal consumidas: ${totalKcal}/${goal}. Balance: ${balance > 0 ? '+' : ''}${balance} kcal. Proteína: ${totalProt}g/${protGoal}g.`
      const msg = await callAI(
        [{ role: 'user', content: ctx }],
        'Eres un coach de hipertrofia de élite. Da feedback ultra específico en 2-3 líneas. Español, directo. Si son <12h y 0 kcal sugiere desayuno hipercalórico. Si >20h y faltan >500 kcal indica urgencia. Si hay exceso menciona qué evitar. Si está en rango motiva brevemente.'
      )
      setOracleMsg(msg)
    } catch (e) { setOracleMsg(`Error: ${e.message}`) }
  ...
  function addWaterManual() {
    const ml = parseInt(waterInput); if (ml > 0) { addWater(ml); setWaterInput('') }
  }

  function addActivity() {
    if (!actInput.trim()) return
    setActivities(a => [...a, { id: Date.now(), text: actInput }]); setActInput('')
  }
  ...
  return (
    <div>
      <Panel>
        <SectionLabel>Balance Calórico</SectionLabel>
        <div style={{ textAlign: 'center', marginBottom: 10 }}>
          <span style={{ fontFamily: 'Inter,sans-serif', fontSize: 52, fontWeight: 700, color: barColor, lineHeight: 1 }}>{totalKcal}</span>
          <span style={{ color: '#888', fontFamily: 'Inter,sans-serif', fontSize: 18 }}> / {goal} kcal</span>
        </div>
        <Bar2 value={totalKcal} max={goal} color={barColor} height={18} />
        <div style={{ textAlign: 'center', marginTop: 8, fontFamily: 'Cormorant Garamond,serif', fontStyle: 'italic', fontSize: 17, color: balance > 0 ? C.red : balance < 0 ? C.blue : C.gold }}>
          {balance > 0 ? `Exceso de ${balance} kcal` : balance < 0 ? `Déficit de ${Math.abs(balance)} kcal` : 'Exactamente en meta'}
        </div>
        <GoldBtn onClick={consultOracle} disabled={oracleLoading} style={{ width: '100%', marginTop: 14 }}>
          {oracleLoading ? 'Consultando...' : '⚡ Consultar Oráculo'}
        </GoldBtn>
        {oracleMsg && (
          <div style={{ marginTop: 12, padding: 14, background: C.stone, borderRadius: 8, borderLeft: `3px solid ${C.gold}`, fontFamily: 'Cormorant Garamond,serif', fontStyle: 'italic', fontSize: 16, color: C.marble, lineHeight: 1.7 }}>
            {oracleMsg}
          </div>
        )}
      </Panel>
      ...
      <Panel>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 }}>
          <span style={{ fontFamily: 'Inter,sans-serif', fontSize: 32, fontWeight: 700, color: C.blue }}>{(water / 1000).toFixed(2)} L</span>
          <span style={{ fontFamily: 'Inter,sans-serif', fontSize: 13, color: '#888' }}>meta 3.0 L</span>
        </div>
        <Bar2 value={water} max={waterGoal} color={C.blue} height={14} />
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
          <input value={waterInput} onChange={e => setWaterInput(e.target.value)} placeholder="ml manual" type="number"
            style={{ flex: 1, background: C.stone, border: `1px solid ${C.gold}40`, borderRadius: 6, color: C.marble, padding: '8px 12px', fontFamily: 'Inter,sans-serif', fontSize: 13 }} />
          <GoldBtn onClick={addWaterManual}>Agregar</GoldBtn>
        </div>
      </Panel>

      <Panel>
        <SectionLabel>Actividad del día</SectionLabel>
        {activities.length === 0
          ? <div style={{ fontFamily: 'Cormorant Garamond,serif', fontStyle: 'italic', color: '#666', fontSize: 15 }}>Sin actividades registradas hoy.</div>
          : activities.map(a => (
            <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: `1px solid ${C.stone}` }}>
              <span style={{ color: C.marble, fontFamily: 'Inter,sans-serif', fontSize: 14 }}>{a.text}</span>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
          <input value={actInput} onChange={e => setActInput(e.target.value)} placeholder="Nueva actividad..."
            onKeyDown={e => e.key === 'Enter' && addActivity()}
            style={{ flex: 1, background: C.stone, border: `1px solid ${C.gold}40`, borderRadius: 6, color: C.marble, padding: '8px 12px', fontFamily: 'Inter,sans-serif', fontSize: 13 }} />
          <GoldBtn onClick={addActivity}>+</GoldBtn>
        </div>
      </Panel>

      <Panel>
        <SectionLabel>Peso del día</SectionLabel>
        <div style={{ display: 'flex', gap: 20, marginBottom: 12, flexWrap: 'wrap' }}>
          {todayWeight && (
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontFamily: 'Inter,sans-serif', fontSize: 36, fontWeight: 700, color: C.gold }}>{todayWeight}</div>
              <div style={{ fontFamily: 'Cinzel,serif', fontSize: 9, color: '#888', letterSpacing: 2 }}>KG HOY</div>
            </div>
          )}
          {weightDiff !== null && (
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontFamily: 'Cinzel,serif', fontSize: 9, color: '#888', letterSpacing: 2 }}>VS AYER</div>
            </div>
          )}
          {prevWeight && (
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontFamily: 'Inter,sans-serif', fontSize: 22, fontWeight: 600, color: '#666' }}>{prevWeight[1]}</div>
              <div style={{ fontFamily: 'Cinzel,serif', fontSize: 9, color: '#888', letterSpacing: 2 }}>ANTERIOR</div>
            </div>
          )}
        </div>
...
function TabNutricion({ profile, foods, setFoods }) {
  ...
  return (
    <div>
      <Panel>
        ...
        <div style={{ marginBottom: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <span style={{ fontFamily: 'Inter,sans-serif', fontSize: 12, color: C.marble }}>Calorías: <b style={{ color: C.gold }}>{totalKcal}</b> / {goal} kcal</span>
            <span style={{ fontFamily: 'Inter,sans-serif', fontSize: 12, color: C.gold }}>{goal > 0 ? Math.round((totalKcal / goal) * 100) : 0}%</span>
          </div>
          <Bar2 value={totalKcal} max={goal} color={C.gold} height={10} />
        </div>
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <span style={{ fontFamily: 'Inter,sans-serif', fontSize: 12, color: C.marble }}>Proteína: <b style={{ color: '#e74c3c' }}>{totalProt}g</b> / {protGoal}g</span>
            <span style={{ fontFamily: 'Inter,sans-serif', fontSize: 12, color: '#e74c3c' }}>{protGoal > 0 ? Math.round((totalProt / protGoal) * 100) : 0}%</span>
          </div>
          <Bar2 value={totalProt} max={protGoal} color='#e74c3c' height={10} />
        </div>
        {totalSodAll > 1500 && (
          <div style={{ marginTop: 10, padding: '8px 12px', background: `${C.red}20`, border: `1px solid ${C.red}`, borderRadius: 6, color: C.red, fontFamily: 'Inter,sans-serif', fontSize: 12 }}>
      ...
      <Panel>
        <SectionLabel>Registrar comida</SectionLabel>
        <textarea value={input} onChange={e => setInput(e.target.value)} placeholder="Describe lo que comiste..."
          style={{ width: '100%', minHeight: 80, background: C.stone, border: `1px solid ${C.gold}40`, borderRadius: 6, color: C.marble, padding: '10px 12px', fontFamily: 'Cormorant Garamond,serif', fontSize: 16, resize: 'vertical' }} />
        <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
        ...
        {aiResult && !aiResult.error && (
          <div style={{ marginTop: 14, background: C.stone, borderRadius: 8, padding: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              <span style={{ fontFamily: 'Cinzel,serif', fontSize: 12, color: C.gold }}>{aiResult.meal_type}</span>
      ...
      {foods.length > 0 && (
        <Panel>
          <SectionLabel>Registros del día</SectionLabel>
          {foods.map(f => (
            <div key={f.id} style={{ borderBottom: `1px solid ${C.stone}`, paddingBottom: 10, marginBottom: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
                  <span style={{ fontFamily: 'Cinzel,serif', fontSize: 10, color: C.gold }}>{f.meal_type}</span>
                  <span style={{ fontFamily: 'Inter,sans-serif', fontSize: 12, color: '#888', marginLeft: 8 }}>{f.kcal} kcal · {f.protein}g prot</span>
              ...
              <div style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 14, color: '#777', fontStyle: 'italic', marginTop: 2 }}>{f.description}</div>
            </div>
          ))}
          <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 4 }}>
            <span style={{ fontFamily: 'Cinzel,serif', fontSize: 11, color: C.gold }}>Total del día</span>
...
function TabAtleta({ profile, exercises, setExercises, sessions, setSessions }) {
  ...
  const iStyle = { background: C.stone, border: `1px solid ${C.gold}40`, borderRadius: 6, color: C.marble, padding: '8px 12px', fontFamily: 'Inter,sans-serif', fontSize: 13, width: '100%' }
    ...
    <div>
      <Panel>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontFamily: 'Cinzel,serif', fontSize: 24, color: C.gold, fontWeight: 700 }}>{trainingDay}</div>
            <div style={{ fontFamily: 'Cormorant Garamond,serif', fontStyle: 'italic', fontSize: 13, color: '#777', marginTop: 2 }}>{fmtDate()}</div>
          </div>
      ...
      <Panel>
        ...
        <div style={{ marginBottom: 8 }}>
          <input value={form.name} onChange={e => fInp('name', e.target.value)} placeholder="Nombre del ejercicio" style={iStyle} />
        </div>
        ...
        {transcript && <div style={{ marginTop: 8, fontFamily: 'Cormorant Garamond,serif', fontStyle: 'italic', fontSize: 14, color: '#777' }}>{transcript}</div>}
      </Panel>

      {exercises.length > 0 && (
        <Panel>
          <SectionLabel>Ejercicios registrados</SectionLabel>
          {exercises.map(e => (
            <div key={e.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: `1px solid ${C.stone}` }}>
              <div>
          ...
          <div style={{ paddingTop: 8 }}>
            <span style={{ fontFamily: 'Cinzel,serif', fontSize: 10, color: C.gold }}>VOLUMEN TOTAL </span>
      ...
      <Panel>
        <SectionLabel>Evaluación IA</SectionLabel>
        ...
        {evaluation && (
          <div style={{ marginTop: 14 }}>
            {evaluation.split('\n').map((line, i) => {
              ...
              return (
                <div key={i} style={{ fontFamily: isH ? 'Cinzel,serif' : 'Cormorant Garamond,serif', fontSize: isH ? 11 : 15, color: isH ? C.gold : C.marble, letterSpacing: isH ? 2 : 0, lineHeight: 1.7, marginTop: isH ? 12 : 0 }}>
                  {line}
                </div>
              )
            })}
      ...
      <Panel>
        <SectionLabel>Calculadora 1RM — Epley</SectionLabel>
        ...
        {rm1 && (
          <>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'Inter,sans-serif', fontSize: 13 }}>
              ...
              <tbody>
                {rmTable.map(r => (
                  <tr key={r.pct} style={{ borderBottom: `1px solid ${C.stone}` }}>
      ...
      {sessions.length > 0 && (
        <Panel>
          ...
          {sessions.slice(0, 5).map(s => (
            <div key={s.id} style={{ borderBottom: `1px solid ${C.stone}`, paddingBottom: 10, marginBottom: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                <span style={{ fontFamily: 'Cinzel,serif', fontSize: 12, color: C.gold }}>{s.type}</span>
                <span style={{ fontFamily: 'Inter,sans-serif', fontSize: 11, color: '#888' }}>{s.date}</span>
              </div>
              {s.exercises.map((e, i) => (
                <div key={i} style={{ fontFamily: 'Inter,sans-serif', fontSize: 12, color: '#aaa', marginBottom: 2 }}>
...
function TabProgreso({ profile, foods: todayFoods, water: todayWater, weights, sessions }) {
  ...
  return (
    <div>
      <Panel>
        <SectionLabel>Stats personales</SectionLabel>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {[{ l: 'Peso', v: `${profile.weight} kg` }, { l: 'Talla', v: `${profile.height} cm` }, { l: 'TMB', v: `${bmr} kcal` }, { l: 'TDEE ajustado', v: `${goal} kcal` }].map(s => (
            <div key={s.l} style={{ background: C.stone, borderRadius: 8, padding: '10px 12px', textAlign: 'center' }}>
              <div style={{ fontFamily: 'Cinzel,serif', fontSize: 9, color: C.gold, letterSpacing: 2, marginBottom: 4 }}>{s.l.toUpperCase()}</div>
      ...
      {weightData.length > 0 && (
        <Panel>
          <SectionLabel>Peso — últimos 7 días</SectionLabel>
          ...
          {monthlyChange && (
            <div style={{ marginTop: 8, textAlign: 'center', fontFamily: 'Inter,sans-serif', fontSize: 13, color: parseFloat(monthlyChange.diff) < 0 ? C.green : parseFloat(monthlyChange.diff) > 0 ? C.red : C.gold }}>
      ...
      <Panel>
        <SectionLabel>Calorías — últimos 7 días</SectionLabel>
      ...
      <Panel>
        <SectionLabel>Hidratación — últimos 7 días</SectionLabel>
        <ResponsiveContainer width="100%" height={140}>
          <BarChart data={waterData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke={C.stone} />
            <XAxis dataKey="day" tick={{ fill: '#888', fontSize: 11 }} />
            <YAxis tick={{ fill: '#888', fontSize: 11 }} tickFormatter={v => `${v}L`} />
            <Tooltip {...ttStyle} formatter={v => [`${v}L`, 'Agua']} />
            <Bar dataKey="agua" fill={C.blue} radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </Panel>
            ...
            <div key={i} style={{ textAlign: 'center', padding: '4px 2px', borderRadius: 5, background: d ? d.bg : 'transparent', border: d?.isToday ? `1px solid ${C.gold}` : '1px solid transparent', minHeight: 34 }}>
              {d && (
                <>
                  ...
                  {d.kcal > 0 && <div style={{ fontFamily: 'Inter,sans-serif', fontSize: 8, color: '#888' }}>{d.kcal}</div>}
                </>
              )}
            </div>
          ))}
      ...
      <Panel>
        <SectionLabel>Memoria visual</SectionLabel>
        <input value={photoNote} onChange={e => setPhotoNote(e.target.value)} placeholder="Nota opcional..."
          style={{ width: '100%', background: C.stone, border: `1px solid ${C.gold}40`, borderRadius: 6, color: C.marble, padding: '8px 12px', fontFamily: 'Inter,sans-serif', fontSize: 13, marginBottom: 8 }} />
        <label>
          <div style={{ background: C.stone, border: `1px dashed ${C.gold}60`, borderRadius: 8, padding: 12, textAlign: 'center', cursor: 'pointer', fontFamily: 'Cinzel,serif', fontSize: 11, color: C.gold, letterSpacing: 1 }}>
            + SUBIR FOTO
          </div>
            ...
            {photos.map(p => (
              <div key={p.id} style={{ position: 'relative', borderRadius: 8, overflow: 'hidden', background: C.stone }}>
                ...
                <div style={{ padding: '5px 8px', background: C.panel + 'ee' }}>
                  ...
                  {p.note && <div style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 12, color: '#aaa', fontStyle: 'italic' }}>{p.note}</div>}
                </div>
      ...
      <Panel>
        <SectionLabel>Oráculo IA</SectionLabel>
...
function ConfigOverlay({ profile, setProfile, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#000000cc', zIndex: 1000, overflowY: 'auto', display: 'flex', justifyContent: 'center', paddingTop: 20 }}>
      <div style={{ background: C.panel, borderRadius: 16, width: '100%', maxWidth: 460, margin: '0 16px 60px', border: `1px solid ${C.gold}40`, alignSelf: 'flex-start' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: `1px solid ${C.stone}` }}>
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#888', fontSize: 26, cursor: 'pointer', lineHeight: 1 }}>×</button>
        </div>
        <div style={{ padding: '16px 20px' }}>
          ...
          <div style={{ background: C.stone, borderRadius: 8, padding: 14, marginBottom: 16 }}>
            ...
            <div style={{ fontFamily: 'Inter,sans-serif', fontSize: 24, fontWeight: 700, color: C.marble }}>{calGoal(form)} <span style={{ fontSize: 13, color: '#888' }}>kcal/día</span></div>
          </div>
...
function RitualesOverlay({ onClose }) {
  ...
  const iStyle = { background: C.stone, border: `1px solid ${C.gold}40`, borderRadius: 6, color: C.marble, padding: '8px 12px', fontFamily: 'Inter,sans-serif', fontSize: 13 }
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#000000cc', zIndex: 1000, overflowY: 'auto', display: 'flex', justifyContent: 'center', paddingTop: 20 }}>
      <div style={{ background: C.panel, borderRadius: 16, width: '100%', maxWidth: 460, margin: '0 16px 60px', border: `1px solid ${C.gold}40`, alignSelf: 'flex-start' }}>
        ...
        <div style={{ padding: '16px 20px' }}>
          ...
          {rituals.length === 0 && (
            <div style={{ fontFamily: 'Cormorant Garamond,serif', fontStyle: 'italic', color: '#666', fontSize: 15, marginBottom: 16 }}>Sin rituales aún.</div>
          )}
            ...
            <div key={r.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 0', borderBottom: `1px solid ${C.stone}` }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontFamily: 'Cinzel,serif', fontSize: 12, color: checks[r.id] ? '#666' : C.marble, textDecoration: checks[r.id] ? 'line-through' : 'none' }}>{r.title}</span>
                  <span style={{ fontFamily: 'Inter,sans-serif', fontSize: 10, color: '#777', marginLeft: 'auto' }}>{r.moment}</span>
                </div>
                {r.desc && <div style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 13, color: '#888', fontStyle: 'italic', marginTop: 2 }}>{r.desc}</div>}
              </div>
          ...
          <input value={newR.desc} onChange={e => setNewR(p => ({ ...p, desc: e.target.value }))} placeholder="Descripción..."
            style={{ ...iStyle, width: '100%', marginBottom: 8, display: 'block' }} />
          <select value={newR.moment} onChange={e => setNewR(p => ({ ...p, moment: e.target.value }))}
            style={{ ...iStyle, width: '100%', marginBottom: 10, display: 'block' }}>
...
function InstruccionesOverlay({ onClose }) {
  ...
  const items = [
    { t: 'Cómo usar HOY', c: 'La pestaña HOY muestra tu balance calórico con barra visual dinámica (dorado=en rango, rojo=exceso, azul=déficit). Registra hidratación con los botones rápidos, anota actividades físicas adicionales y registra tu peso diario. El Oráculo da feedback inmediato según tu progreso del día.' },
        ...
        <div style={{ padding: '8px 20px 20px' }}>
          {items.map((item, i) => (
            <div key={i} style={{ borderBottom: `1px solid ${C.stone}` }}>
              ...
              {open === i && (
                <div style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 15, color: '#aaa', lineHeight: 1.7, paddingBottom: 14 }}>{item.c}</div>
              )}
            </div>
          ))}
        </div>
...
export default function App() {
  ...
  return (
    <div style={{ background: C.black, minHeight: '100vh', color: C.marble, fontFamily: 'Inter,sans-serif', paddingBottom: 80 }}>
      <div style={{ maxWidth: 480, margin: '0 auto', position: 'relative' }}>

        {/* HEADER */}
        <div style={{ position: 'sticky', top: 0, zIndex: 100, background: C.black, borderBottom: `1px solid ${C.stone}`, padding: '12px 16px 0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
            <div>
              <div style={{ fontFamily: '"Cinzel Decorative",cursive', fontSize: 20, color: C.gold, fontWeight: 700, letterSpacing: 1 }}>
                OLYMPUS <span style={{ fontSize: 11, opacity: 0.6, fontFamily: 'Cinzel,serif' }}>v2.0</span>
              </div>
              <div style={{ fontFamily: '"Cormorant Garamond",serif', fontStyle: 'italic', fontSize: 14, color: '#777', marginTop: 1 }}>
                {getGreeting(profile.name)} · {fmtDate()}
              </div>
            </div>
            <div style={{ position: 'relative' }}>
              <button onClick={() => setMenuOpen(p => !p)}
                style={{ background: C.stone, border: `1px solid ${C.gold}40`, borderRadius: 8, padding: '8px 11px', cursor: 'pointer', fontSize: 15, color: C.marble }}>
                ⚙️
              </button>
              {menuOpen && (
                <div style={{ position: 'absolute', right: 0, top: 'calc(100% + 4px)', background: C.panel, border: `1px solid ${C.gold}40`, borderRadius: 8, overflow: 'hidden', minWidth: 170, zIndex: 200, boxShadow: '0 8px 24px #00000088' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 5, marginBottom: 10 }}>
            {[
              { l: 'KCAL', v: `${totalKcal}/${goal}` },
              { l: 'PROT', v: `${totalProt}/${protGoal}g` },
              { l: 'AGUA', v: `${(water / 1000).toFixed(1)}L` },
              { l: 'HOY', v: trainingDay.length > 7 ? trainingDay.substring(0, 7) : trainingDay },
            ].map(s => (
              <div key={s.l} style={{ background: C.stone, borderRadius: 6, padding: '5px 4px', textAlign: 'center' }}>
                <div style={{ fontFamily: 'Cinzel,serif', fontSize: 8, color: C.gold, letterSpacing: 1 }}>{s.l}</div>
                <div style={{ fontFamily: 'Inter,sans-serif', fontSize: 10, color: C.marble, fontWeight: 600, marginTop: 1 }}>{s.v}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* BOTTOM TABS */}
        <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 480, background: C.black, borderTop: `1px solid ${C.stone}`, display: 'flex', zIndex: 100 }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              style={{ flex: 1, padding: '10px 4px 12px', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, borderTop: tab === t.id ? `2px solid ${C.gold}` : '2px solid transparent' }}>
              <span style={{ fontSize: 20 }}>{t.icon}</span>
              <span style={{ fontFamily: 'Cinzel,serif', fontSize: 8, color: tab === t.id ? C.gold : '#666', letterSpacing: 1 }}>{t.label}</span>
            </button>
          ))}
        </div>
